...
* ngx-translate-router
[![npm version](https://badge.fury.io/js/ngx-translate-router.svg)](https://badge.fury.io/js/ngx-translate-router)
...
Library versions are aligned with Angular versions, choose the version that matches your Angular version.
  ...
  - [Usage](#usage)
    ...
    - [How it works](#how-it-works)
      ...
      - [WildCard Path](#wildcard-path)
        - [Favored way](#favored-way)
        - [Alternative](#alternative)
      - [Matcher params translation](#matcher-params-translation)
        - [Configure routes](#configure-routes)
        - [Small changes to your matcher](#small-changes-to-your-matcher)
        - [Matcher params translated without localizeMatcher issue](#matcher-params-translated-without-localizematcher-issue)
...
```
npm install --save ngx-translate-router
```
...
In order to use Http loader for config files, you must include `ngx-translate-router-http-loader` package and use its `LocalizeRouterHttpLoader`. 
...
Apart from providing routes which are mandatory, and parser loader you can provide additional configuration for more granular setting of `ngx-translate-router`. More information at [LocalizeRouterConfig](#localizerouterconfig). 

### Server side

In order to use `ngx-translate-router` in Angular universal application (SSR) you need to:

1. [Initialize the module](#initialize-module)
2. In case you opted for initializing with Http loader, you need to take care of static file location. `ngx-translate-router-http-loader` by default will try loading the config file from `assets/locales.json`. This is a relative path which won't work with SSR. You could use one of the following approaches,
...
`ngx-translate-router` intercepts Router initialization and translates each `path` and `redirectTo` path of Routes.
...
Upon every route change `ngx-translate-router` kicks in to check if there was a change to language. Translated routes are prepended with two letter language code:
```
...
#### Methods:
- `forRoot(routes: Routes, config: LocalizeRouterConfig = {}): ModuleWithProviders`: Main initializer for ngx-translate-router. Can provide custom configuration for more granular settings.
...
<a href="https://github.com/ruizmarc/ngx-translate-router/graphs/contributors">
  <img src="https://contributors-img.web.app/image?repo=ruizmarc/ngx-translate-router" />
</a>
